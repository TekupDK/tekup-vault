...
async function testPooler(dbUrl, host) {
  ...
  try {
    const r = await pool.query('SELECT version()');
    console.log('✅ Session Pooler (6543) OK:', r.rows[0].version.split('\n')[0]);
  } catch (e) {
...
(async () => {
  const dbUrl = process.env.DATABASE_URL || process.env.SUPABASE_DB_URL || process.env.POSTGRES_URL || process.env.VAULT_DATABASE_URL || process.argv.find(a => a.startsWith('db='))?.split('=')[1];
  if (!dbUrl) {
    console.error('Missing database URL. Provide via DATABASE_URL (preferred) or SUPABASE_DB_URL/POSTGRES_URL/VAULT_DATABASE_URL, or pass as CLI arg: db=postgresql://...');
    process.exit(1);
  }
